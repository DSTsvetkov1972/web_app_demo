    ...
    with conn:
        ...
        if task_status=='queued':
            sql = (f"INSERT INTO tasks "
                f"(task_id, user_id, file_name, task_timestamp, task_status, task_status_timestamp) "
                f"VALUES ({max_task_id+1}, {user_id}, '{file_name}', '{datetime.now()}','queued', '{datetime.now()}')")
        else:
            sql = (f"INSERT INTO tasks "
                f"(task_id, user_id, file_name, task_timestamp, start_processing_timestamp, task_progress_timestamp, task_status, task_status_timestamp, task_rem) "
                f"VALUES ({max_task_id+1}, {user_id}, '{file_name}', '{datetime.now()}', '{datetime.now()}', '{datetime.now()}', 'error', '{datetime.now()}',  'Already was puted in queue')")            
        cur.execute(sql)
...
def get_all_tasks_ids():
    ...
    with conn:
        cur = conn.cursor()
        sql = f"SELECT task_id FROM tasks WHERE task_status IN ('processing', 'queued') ORDER BY task_id"
        cur.execute(sql)
        res = cur.fetchall()
    if res:
        return res
...
def reset_processing_to_queued():
    ...
    with conn:
        cur = conn.cursor()
        sql = f"UPDATE tasks SET task_status='queued' WHERE task_status='processing'"
        cur.execute(sql)        
