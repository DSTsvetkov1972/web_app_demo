...
from db_fns import init_db, create_user, put_task_in_queue, get_first_taskid_id_in_queue, get_tasks_ids, get_task_info, reset_processing_to_queued, set_db_column_value, get_all_tasks_ids
...
def files_uploader(user_id):
    with st.sidebar: 
        with st.expander(f'Загрузите файлы для обработки:'):
            uploaded_files = st.file_uploader("Загрузите файлы для обработки", accept_multiple_files=True, label_visibility="collapsed")#, accept_multiple_files=False)
            if uploaded_files != []:
            ...
            with placeholder.container(border=True):
                if tasks_ids:
                    for task_id in tasks_ids[:5]:
                        ...
                        st.header(f"**Задача: {task_info['user_id']}-{task_info['task_id']}**")
                        if task_info['task_status']=='processing':

                            if task_info['task_progress']:
                                st.progress(int(task_info['task_progress']))
                            st.html(f'<b>В очереди с:</b><br><font color="grey">{task_info["task_timestamp"][:19]}<br><b><font color="black">Обработка стартовала:</b><br><font color="grey">{task_info["task_timestamp"][:19]}')
                            #st.html(f'<font color="grey">{task_info["task_timestamp"][:19]}<br><b><font color="black">Обработка стартовала:</b><br><font color="grey">{task_info["task_timestamp"][:19]}')                            
                        else:
                    ...
                    if task_info['task_status'] != 'complited':
                        with st.container(border=True):
                            ...
                            if task_info['task_status'] == 'error':                    
                                ...
                                with col2:
                                    with open(os.path.join(os.getcwd(), 'data', f"{current_user_id}", 'output', f"{task_info['file_name']}.docx"), 'rb') as f:
                                        st.download_button("📥 Загрузить результат",  f, use_container_width=True, key=f'ready_download_{task_info["task_id"]}', file_name=f'{task_info["file_name"]}.docx')
